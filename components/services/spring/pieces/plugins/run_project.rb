...
#        cmd = "java -Xrunjdwp:transport=dt_socket,address=8787,server=y,suspend=n -javaagent:#{agentjar} -javaagent:lib/optional/spring-agent.jar -cp #{cp} #{props} org.appcelerator.endpoint.HTTPEndpoint #{port} \"#{webdir}\" \"#{servicesdir}\" \"#{tmpdir}\" #{scanperiod}"
        cmd = "java -javaagent:#{agentjar} -javaagent:lib/optional/spring-agent.jar -cp #{cp} #{props} org.appcelerator.endpoint.HTTPEndpoint #{port} \"#{webdir}\" \"#{servicesdir}\" \"#{tmpdir}\" #{scanperiod}"
